    ...
    public class GroupChatHub : Hub
    {
        ...
        public async Task SendMessageToGroup(string message)
        {
            try
            {
                ...
                {
                    await Clients.Caller.SendAsync("Error", "您尚未加入群组");
                    return;
                }
                ...
                _logger.LogInformation($"消息已推送到群组 {userConnection.RoomId}");
            }
            catch (Exception ex)
            {
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            try
            {
                if (_connections.TryRemove(Context.ConnectionId, out var userConnection))
                {
                    ...
                    _logger.LogInformation($"用户 {userConnection.Username}({userConnection.UserId}) 离开群组 {userConnection.RoomId}");
                }
            }
            catch (Exception ex)
            {
